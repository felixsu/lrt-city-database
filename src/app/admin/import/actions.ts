...
import { normalizeCurrency } from "@/lib/consumer-analytics";
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const row of rows) {
        ...
        await tx.ownershipDocument.create({ data: { userId: user.id, unitNumber: row.unitNumber.trim(), accountNumber: accountNumber(row.ppjbNumber), sppuNumber: row.sppuNumber.trim() || null, purchasePrice: row.purchasePrice || null, paymentType: isRefund ? row.paymentType || null : null, loanBankName: isRefund ? row.loanBankName || null : null, loanTenorMonths: row.loanTenorMonths, loanMonthsPaid: row.loanMonthsPaid, loanPaymentStatus: isRefund ? row.loanPaymentStatus || null : null, tuntutan: row.tuntutan || null, materialLossPaid: isRefund ? normalizeCurrency(row.materialLossPaid) : null, materialDetails: row.materialDetails || null, remainingArrears: row.remainingArrears || null, otherLosses: isRefund ? normalizeCurrency(row.otherLosses) : null, lossBasisCalc: row.lossBasisCalc || null, pinjamPakai: row.pinjamPakai || null, maxWaitDuration: row.maxWaitDuration || null, compensation: row.compensation || null, surveyTimestamp: parseTimestamp(row.timestamp) } });
      }
    });
